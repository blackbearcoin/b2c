...
#if defined(NDEBUG)
#error "B2C cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 300 * 60;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in ublackbearcoin) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 100 && nHeight >= 0) {
        nSubsidy = 48250 * COIN;
    } else if (nHeight <= 124999 && nHeight >= 101) { 
      nSubsidy = 3 * COIN;
    } else if (nHeight <= 199999 && nHeight >= 125000) { 
      nSubsidy = 2 * COIN;
    } else {
      nSubsidy = 1 * COIN;
    }
...
{
    RenameThread("blackbearcoin-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateB2CSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // B2C: We use certain sporks during IBD, so check to see if they are
