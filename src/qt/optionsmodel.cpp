...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::B2C);
    ...
    if (!settings.contains("nAnonymizeB2CAmount"))
        settings.setValue("nAnonymizeB2CAmount", 1000);

    nAnonymizeB2CAmount = settings.value("nAnonymizeB2CAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeB2CAmount"))
        SoftSetArg("-anonymizeblackbearcoinamount", settings.value("nAnonymizeB2CAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in blackbearcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeB2CAmount:
            return QVariant(nAnonymizeB2CAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeB2CAmount:
            nAnonymizeB2CAmount = value.toInt();
            settings.setValue("nAnonymizeB2CAmount", nAnonymizeB2CAmount);
            emit anonymizeB2CAmountChanged(nAnonymizeB2CAmount);
            break;
        case CoinControlFeatures:
