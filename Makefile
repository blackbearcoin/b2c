...
pkgdatadir = $(datadir)/blackbearcoin
pkgincludedir = $(includedir)/blackbearcoin
pkglibdir = $(libdir)/blackbearcoin
pkglibexecdir = $(libexecdir)/blackbearcoin
...
CONFIG_HEADER = $(top_builddir)/src/config/blackbearcoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/blackbearcoin-config.h.in \
...
ACLOCAL = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/missing aclocal-1.16
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/missing automake-1.16
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = blackbearcoin-cli
BITCOIN_DAEMON_NAME = blackbearcoind
BITCOIN_GUI_NAME = blackbearcoin-qt
BITCOIN_TX_NAME = blackbearcoin-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = blackbearcoin
PACKAGE_BUGREPORT = blackbearcoin.io
PACKAGE_NAME = Black Bear Coin Core
PACKAGE_STRING = Black Bear Coin Core 1.0.0
PACKAGE_TARNAME = blackbearcoin
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /opt/bitcoin-factory/works/bitcoin/blackbearcoin
abs_srcdir = /opt/bitcoin-factory/works/bitcoin/blackbearcoin
abs_top_builddir = /opt/bitcoin-factory/works/bitcoin/blackbearcoin
abs_top_srcdir = /opt/bitcoin-factory/works/bitcoin/blackbearcoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /opt/bitcoin-factory/works/bitcoin/blackbearcoin/build-aux/install-sh
...
empty := 
space := $(empty) $(empty)
OSX_APP = BlackbearCoin-Core.app
...
DIST_CONTRIB = $(top_srcdir)/contrib/blackbearcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/blackbearcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/blackbearcoind.bash-completion \
...
COVERAGE_INFO = baseline.info \
  test_blackbearcoin_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_blackbearcoin_coverage.info test_blackbearcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BlackbearCoin-Core $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/blackbearcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/blackbearcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/blackbearcoin-config.h
$(top_srcdir)/src/config/blackbearcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/blackbearcoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/BlackbearCoin-Core: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BlackbearCoin-Core
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BlackbearCoin-Core: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_blackbearcoin.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_blackbearcoin -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_blackbearcoin_filtered.info: test_blackbearcoin.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_blackbearcoin_filtered.info
...
#test_blackbearcoin_coverage.info: baseline_filtered.info test_blackbearcoin_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_blackbearcoin_filtered.info -o $@

#total_coverage.info: test_blackbearcoin_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_blackbearcoin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_blackbearcoin.coverage/.dirstamp:  test_blackbearcoin_coverage.info
...
#cov: test_blackbearcoin.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_blackbearcoin.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
