...
B2C now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<blackbearcoin_address\> \<monthly_payment_blackbearcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ B2C, _12_ times over the course of a year totaling _24000_ B2C.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<blackbearcoin_address\> \<monthly_payment_blackbearcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` B2C.
 ...
 * vote-many          - Vote on a B2C initiative
 * vote-alias         - Vote on a B2C initiative
 * vote               - Vote on a B2C initiative/budget
